    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-8 py-6 sm:py-10">
      ...
      {loadError ? (
        <div className="rounded-2xl border border-[#E5E7EB] p-8 bg-[#FEF2F2] text-[#B91C1C]">
          <div className="font-semibold">Challenges unavailable right now.</div>
          <div className="mt-2 text-sm text-[#92400E]">{loadError}</div>
          <div className="mt-3 text-sm text-[#92400E]">Configure your database or use local SQLite to enable challenges.</div>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {challenges.map((c) => (
            <Link key={c.id} href={`/challenges/${c.id}`} className="rounded-2xl border border-[#E5E7EB] p-5 hover:border-[#111] hover:shadow-sm transition bg-white">
              <div className="flex items-start justify-between gap-2 mb-3">
                ...
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-mono ${statusBadgeClass(c.status)}`}>
                  {c.status}
                </span>
              </div>
              ...
              {c.requirements.length > 0 && (
                <div className="mt-3 pt-3 border-t border-[#E5E7EB] flex flex-wrap gap-1">
                  {c.requirements.slice(0, 3).map((r: any) => (
